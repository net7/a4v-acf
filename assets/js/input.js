...
(function($, undefined){
	...
	var Field = acf.Field.extend({
		...
		events: {
			...
			'click [data-name="add_a4v_item"]': 	'onClickAddA4vItem'
		},
		...
		onClickAddA4vItem: function( e, $el ){
			if($($el).data("name") != "add_a4v_item") return;
			e.preventDefault();
			e.stopPropagation();
			$el.addClass('disabled');
			// vars
			var val = $($el).parents("li").eq(0).find("input[type=hidden]").val();		
			...
			// extra
			ajaxData.action = 'acf/fields/a4v/addItem';
			ajaxData.field_key = this.get('key');
			ajaxData.value = val;
					
			var onSuccess = function( json ){
				if(json['post_id'] && parseInt(json['post_id'])){
					$($el).data("postid", json['post_id']  );
					$($el).data("name", "edit_item"  );
					$($el).trigger("click");		
		...
		getAjaxData: function(){
			...
			// extra
			ajaxData.action = 'acf/fields/a4v/query';
			ajaxData.field_key = this.get('key');
			...
			var walk = function( data ){
				...
				// is item
					...
					if( data.children !== undefined ) {
						...
						html += '<li><span class="acf-rel-label">' + acf.escHtml( data.label ) + '</span><ul class="acf-bl">';
	...
	*/
	...
	if( typeof acf.add_action !== 'undefined' ) {
		...
		*/
		...
		acf.add_action('ready_field/type=a4v_field', initialize_field);
		acf.add_action('append_field/type=a4v_field', initialize_field);
		...
		*/
		...
		$(document).on('acf/setup_fields', function(e, postbox){
			...
			$(postbox).find('.field[data-field_type="a4v_field"]').each(function(){
